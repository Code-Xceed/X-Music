...
public final class ArtworkRenderer {
     ...
     */
    public static void renderArtwork(GuiGraphicsExtractor g, String artworkUrl, int x, int y, int size) {
        ...
        if (loc != null) {
            g.blit(loc, x, y, size, size, 0f, 0f, 1f, 1f);
            return;
        }

        // Not loaded yet â€” trigger async download if not already in progress
    ...
    public static void renderPlaceholder(GuiGraphicsExtractor g, int x, int y, int size) {
        ...
        int ny = y + (size - 8) / 2;
        g.text(font, note, nx, ny, 0xFF505050, false);
    }
