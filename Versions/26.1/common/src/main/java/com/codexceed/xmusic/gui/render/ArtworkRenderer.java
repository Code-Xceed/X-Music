...
public final class ArtworkRenderer {
     ...
     */
    public static void renderArtwork(GuiGraphicsExtractor g, String artworkUrl, int x, int y, int w, int h) {
        renderArtwork(g, artworkUrl, null, x, y, w, h, 1.0f);
    }

    public static void renderArtwork(GuiGraphicsExtractor g, TrackRef track, int x, int y, int w, int h) {
        renderArtwork(g, track != null ? track.getArtworkUrl() : null, track, x, y, w, h, 1.0f);
    }

    public static void renderArtwork(GuiGraphicsExtractor g, TrackRef track, int x, int y, int w, int h, float alpha) {
        renderArtwork(g, track != null ? track.getArtworkUrl() : null, track, x, y, w, h, alpha);
    }

    public static void renderArtwork(GuiGraphicsExtractor g, String artworkUrl, TrackRef track, int x, int y, int w, int h) {
        renderArtwork(g, artworkUrl, track, x, y, w, h, 1.0f);
    }

    public static void renderArtwork(GuiGraphicsExtractor g, String artworkUrl, TrackRef track, int x, int y, int w, int h, float alpha) {
    ...
    public static void renderPlaceholder(GuiGraphicsExtractor g, int x, int y, int w, int h) {
        renderPlaceholder(g, null, x, y, w, h, 1.0f);
    }

    public static void renderPlaceholder(GuiGraphicsExtractor g, TrackRef track, int x, int y, int w, int h) {
        renderPlaceholder(g, track, x, y, w, h, 1.0f);
    }

    /** Render a beautiful, custom-typed placeholder depending on track metadata! */
    public static void renderPlaceholder(GuiGraphicsExtractor g, TrackRef track, int x, int y, int w, int h, float alpha) {
        ...
        try {
            net.minecraft.world.item.Item item = net.minecraft.world.item.Items.MUSIC_DISC_MALL;

            if (track != null) {
                boolean isDownloaded = DownloadManager.getInstance().getState(track) == DownloadState.COMPLETED;
                boolean isLocal = "local".equals(track.getSourceId());
                if (isDownloaded) {
                    item = net.minecraft.world.item.Items.MUSIC_DISC_FAR;
                } else if (isLocal) {
                    item = net.minecraft.world.item.Items.MUSIC_DISC_BLOCKS;
                } else {
                    item = net.minecraft.world.item.Items.MUSIC_DISC_CHIRP;
                }
            }

            IconRenderer.itemIcon(g, new net.minecraft.world.item.ItemStack(item), x, y, w, h);
        } catch (Throwable ignored) {
            // Components not bound yet during early main menu / loading screen
        }
