...
/**
 * Search tab â€” full-featured search with keyboard nav, queue, history, and more.
 */
public final class SearchTab {
    ...
    private static final int TOGGLE_WIDTH = 140;
    ...
    public void render(GuiGraphicsExtractor graphics, Font font, GuiFrame frame, int mouseX, int mouseY) {
        ...
        String placeholder = isYoutube ? "Type to search..." : "Search official songs...";
        ...
        // Clear button (X) â€” only when query is not empty
        ...
        if (isSearching) {
            ...
            // Result count â€” sticky header (outside scissor so it doesn't scroll)
    ...
    private void renderActionBar(GuiGraphicsExtractor graphics, Font font, int x, int y, int w, int mouseX, int mouseY, GuiFrame frame) {
        ...
        // Duration Filter toggle â€” auto-sized
    ...
    private void renderSourceToggle(GuiGraphicsExtractor graphics, Font font, int x, int y, int width) {
    ...
    public boolean mouseClicked(GuiFrame frame, double mouseX, double mouseY, int button) {
        ...
        if (urlInputOpen && isYoutube) actionY += URL_INPUT_HEIGHT + SECTION_GAP;
        int btnX = searchBarX;
        int btnH = ACTION_BAR_HEIGHT - 4;
        ...
        if (GuiRender.inside(mouseX, mouseY, x, resultListY, w, listH)) {
            ...
            for (int i = 0; i < displayResults.size(); i++) {
                if (GuiRender.inside(mouseX, mouseY, searchBarX, currentY, searchBarW, TrackRow.HEIGHT)) {
                    TrackRef track = displayResults.get(i);
                    // Heart click â€” toggle favorite (any button)
    ...
    public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
        ...
        if (ctrl && keyCode == GLFW.GLFW_KEY_V) {
            ...
            if (clip != null && !clip.isEmpty()) {
                if (urlInputOpen && isYoutube) { urlInput += clip; urlError = null; }
        ...
        if (ctrl && keyCode == GLFW.GLFW_KEY_A) {
            if (urlInputOpen && isYoutube) urlInput = "";
        ...
        // Query focused mode â€” typing goes to search bar
        if (queryFocused) {
        ...
        // Not focused â€” arrow keys and Enter on selected result still work
        List<TrackRef> displayResults = durationFilterOn ? filterByDuration(searchResults) : searchResults;
        if (!displayResults.isEmpty()) {
            if (keyCode == GLFW.GLFW_KEY_DOWN) {
                selectedIndex = selectedIndex < 0 ? 0 : Math.min(selectedIndex + 1, displayResults.size() - 1);
    ...
    private void loadUrl() {
        ...
        ServiceManager.getLavaSearch().search(loadUri)
                ...
                }).exceptionally(error -> { urlLoading = false; urlError = "Failed to load URL"; return null; });
    }
    ...
    // â”€â”€ Context Popup (Add to Playlist / Queue) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
