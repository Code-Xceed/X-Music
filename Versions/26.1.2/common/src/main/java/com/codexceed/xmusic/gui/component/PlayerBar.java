...
public final class PlayerBar {
    ...
    public void render(GuiGraphicsExtractor graphics, Font font, GuiFrame frame, int mouseX, int mouseY) {
        ...
        // 2. Progress Bar (Top Border) - Draggable â€” only for non-NATIVE tracks
        if (!isNativeTrack) {
            int progressW = w;
            int progressX = x;
            int progressY = y;
        ...
        // 5. Draw track info (Left) â€” two-line layout: title on top, artist + action buttons below
        int art = 34;
        int artX = x + 10;
        int artY = y + 22;

        // Music logo box â€” show album art if available, else noteblock icon
        ...
        // Line 1: Title â€” full width available (up to controls)
    ...
    public boolean mouseClicked(GuiFrame frame, double mouseX, double mouseY) {
        ...
        // Progress Bar â€” only for non-NATIVE tracks (REMOTE/EXTERNAL)
        ...
        int rightEdge = x + w - 24;
        int volW = 70;
        int pctSlotW = 27; // fixed: font.width("100%") â‰ˆ 27
        int pctX = rightEdge - pctSlotW;
        int volX = pctX - 8 - volW;
        int volY = y + 54;
    ...
    private boolean clickPlaylistPopup(int barX, int barY, int barW, int barH, TrackRef track, double mx, double my) {
        ...
        // "Create New Playlist" row
        if (creatingNewPlaylist) {
            // Click on the input area â€” no action, just keep focus
    ...
    private void renderIconButtonSmooth(GuiGraphicsExtractor graphics, Font font, int x, int y, int width, int height, boolean active, float hoverLerp, IconDraw icon) {
        ...
        if (active) {
            ...
            // Icon color interpolates: TEXT_SOFT â†’ ACCENT_BRIGHT on hover
